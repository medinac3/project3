...
//FIFO-based page replacement with 128 physical frames 
...
// FIFO page replacement: circular pointer to the oldest loaded frame 
int fifo_ptr = 0;

// Per-frame page mapping so we know which page to invalidate on eviction 
int frame_to_page[FRAMES];
...
// Load a page from backing store; get rid of the oldest frame (FIFO) when physical memory is full 
void handle_page_fault(int page_number, FILE *backing_store) {
    ...
    if (next_free_frame < FRAMES) {
        ...
        // All frames occupied: evict the frame pointed to by fifo_ptr (oldest loaded) 
        target_frame = fifo_ptr;
        ...
        // Advance FIFO pointer to the next oldest frame 
    ...
    // Update page table and frame-to-page map 
